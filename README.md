...
| `find_capability` | `query` (необязательно) | Подбор инструмента: детерминированный отбор со словарём синонимов. При неуверенном отборе и при пустом запросе отдаёт каталог целиком, чтобы выбирала модель агента. |
