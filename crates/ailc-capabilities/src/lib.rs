...
enum SecretKind {
    ...
    /// scope File и склейка литералов, поэтому ловится и секрет, собранный конкатенацией
    /// или разнесённый переносом строки (T04). Высокий порог для коротких значений
    /// отсекает плейсхолдеры; пониженный порог применяется к длинным и низкоалфавитным
    /// значениям, для которых сама длина уже является свидетельством. Построчного
    /// энтропийного вида в таблице нет: после T04 все энтропийные правила многострочные
    /// (движковый `Matcher::entropy` при надобности вернёт его одной строкой).
    MultilineEntropy {
        pattern: &'static str,
        min_bits: f64,
    },
