...
pub fn is_test_path(rel: &str) -> bool {
    // Обратная косая черта приводится к прямой: вызывающие передают сюда относительный
    // путь в родном для платформы виде, и без приведения проверки каталожных форм ниже
    // (`tests/`, `/tests/`) на Windows не срабатывали, тестовые файлы попадали в сканеры
    // и давали находки, которых нет на других платформах.
