//! E7 Store — атомарный файловый CRUD «запись-на-файл» под каталогом `<root>/.ailc/`.
//!
...
/// Проверка одного компонента пути: запрещаем выход из песочницы `.ailc/`.
...
impl Store {
    /// Каталог пространства имён: `<root>/.ailc/<namespace>/`.
    fn namespace_dir(ctx: &Ctx, namespace: &str) -> PathBuf {
        ctx.root.join(".ailc").join(namespace)
    }

    /// Прочитать все файлы из `.ailc/<namespace>/`.
    ...
    /// Атомарно записать/перезаписать файл `.ailc/<namespace>/<name>` (с созданием каталогов).
    ///
    ...
    /// Дописать строку в файл `.ailc/<namespace>/<name>` (для журнала решений).
