...
mod tests {
    ...
    /// Инструменту, собирающему проект, отводится заведомо больше времени, чем обычному:
    /// иначе hard-ось «Тесты» не закрывается на проекте среднего размера ни при каком
    /// качестве кода, а исход зависит от того, была ли сборка тёплой.
    #[test]
    fn сборочный_предел_больше_обычного() {
        assert!(
            BUILD_TIMEOUT_SECS > DEFAULT_TIMEOUT_SECS,
            "предел сборки {BUILD_TIMEOUT_SECS}с обязан превышать обычный {DEFAULT_TIMEOUT_SECS}с"
        );
    }
