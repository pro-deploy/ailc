...
//!
//! ОРИЕНТИР, не юрконсультация: финально проверяет юрист. Конкретные размеры санкций
//! и статьи уголовного кодекса намеренно не приводятся (см. поле `consequence`).
...
struct Ob {
    ...
    law: &'static str,
    what: &'static str,
    /// Класс последствий нарушения, КАЧЕСТВЕННО. Конкретные суммы штрафов и статьи
    /// уголовного кодекса здесь намеренно не приводятся: они меняются с каждой редакцией
    /// нормы, а зашитые в бинарь значения устаревают вместе с ним и превращают ориентир
    /// в недостоверную юридическую справку. Актуальные санкции смотрите в первоисточнике
    /// и у юриста.
...
pub fn run(args: &[String]) {
    ...
    println!(
        "  Размеры санкций и статьи кодексов здесь не приводятся намеренно: они меняются с\n\
         \x20 редакцией нормы. Сверяйтесь с действующей редакцией на pravo.gov.ru.\n"
    );
    ...
    let foreign_infra =
        pdn && ask("3. Используете зарубежные облака/аналитику/CRM/рассылки (AWS, GA, Mailchimp)?");
...
fn obligations(p: &Profile) -> Vec<Ob> {
    let mut o = Vec::new();
    if p.pdn {
        o.push(Ob { risk: 2, law: "152-ФЗ", what: "Уведомить РКН об обработке ПДн ДО старта; политика конфиденциальности; согласие активным opt-in; уведомление об утечке 24ч/72ч", consequence: "административная ответственность; при утечке повышенная, при повторном нарушении оборотный штраф", hint: "[КОД] consent + дизайн (уведомление РКН)" });
        o.push(Ob {
            risk: 2,
            law: "152-ФЗ ст.19",
            what: "Защита ПДн: не логировать в открытом виде, не хранить пароли в plaintext/MD5",
            consequence: "учитывается в составе нарушения при утечке персональных данных",
            hint: "[КОД] pdn-logs, secret, owasp",
        });
    }
    if p.pdn && p.ru_users {
        o.push(Ob {
            risk: 2,
            law: "242-ФЗ",
            what: "Локализация: первичная запись/хранение ПДн граждан РФ — на серверах в РФ",
            consequence: "административная ответственность, при повторном нарушении повышенная",
            hint: "[КОД] localization",
        });
    }
    if p.pdn && p.foreign_infra {
        o.push(Ob {
            risk: 2,
            law: "152-ФЗ ст.12",
            what: "Трансгранична: уведомить РКН ДО передачи ПДн за рубеж; отдельное согласие",
            consequence: "вплоть до уголовной ответственности",
            hint: "[КОД] cross-border",
        });
    }
    if p.biometrics {
        o.push(Ob { risk: 2, law: "572-ФЗ", what: "Биометрию обрабатывать только через ЕБС; собственные биометрические системы запрещены", consequence: "повышенная административная ответственность", hint: "дизайн" });
    }
    if p.registration && p.ru_users {
        o.push(Ob {
            risk: 1,
            law: "ФЗ о связи",
            what: "Идентификация пользователей по номеру телефона РФ; запрет анонимной регистрации",
            consequence: "ограничение доступа к ресурсу либо административная ответственность",
            hint: "дизайн",
        });
    }
    if p.ads {
        o.push(Ob { risk: 2, law: "38-ФЗ", what: "Маркировка рекламы: токен erid через ОРД, пометка «Реклама», отчёт в ЕРИР, 3% сбор", consequence: "административная ответственность", hint: "[КОД] erid (частично) + дизайн" });
    }
    if p.recommender {
        o.push(Ob { risk: 1, law: "149-ФЗ ст.10.2-1", what: "Уведомить пользователей о применении рекомендательных технологий; правила на сайте", consequence: "вплоть до ограничения доступа к ресурсу", hint: "дизайн" });
    }
    if p.payments {
        o.push(Ob {
            risk: 1,
            law: "54-ФЗ",
            what: "ККТ/онлайн-касса, фискальные чеки через ОФД",
            consequence: "административная ответственность, исчисляемая от суммы расчёта",
            hint: "дизайн",
        });
    }
    if p.minors {
        o.push(Ob {
            risk: 1,
            law: "436-ФЗ",
            what: "Возрастная маркировка (0+/6+/12+/16+/18+); защита детей от вредной информации",
            consequence: "административная ответственность",
            hint: "дизайн",
        });
    }
    if p.large_audience {
        o.push(Ob {
            risk: 1,
            law: "РКН-реестры",
            what: "Регистрация в реестре (соцсети 500к+, каналы 10к+ с 01.11.2024)",
            consequence: "запрет распространения рекламы либо ограничение доступа к ресурсу",
            hint: "дизайн",
        });
    }
    if p.foreign_company && p.large_audience {
        o.push(Ob {
            risk: 2,
            law: "236-ФЗ",
            what: "«Приземление»: филиал/представительство в РФ + личный кабинет в РКН",
            consequence: "оборотный штраф",
            hint: "дизайн",
        });
    }
...
fn print_report(p: &Profile, obs: &[Ob]) {
    ...
    if obs.is_empty() {
        ...
        println!("  при сомнениях сверьтесь с юристом и с действующей редакцией нормы.\n");
        return;
    }
    for o in obs {
        let mark = if o.risk == 2 {
            "⚠ ВЫСОКИЙ"
        } else {
            "•  средний"
        };
...
fn tailored_constitution(p: &Profile) -> String {
    ...
    s.push_str("\n# Полный набор правил и пояснения: README.md, раздел «Комплаенс РФ».\n");
    s
}
...
fn personal_checklist(obs: &[Ob]) -> String {
    ...
    for o in obs {
        ...
        s.push_str(&format!(
            "| {r} | {} | {} | {} | {} |\n",
            o.law, o.what, o.consequence, o.hint
        ));
    }
