---
name: generate-sbom
description: "Сгенерировать SBOM (CycloneDX) из lock-файлов проекта — состав зависимостей для CI/supply-chain."
license: Apache-2.0
metadata:
  family: generate
  engine: generator
  tier: core
  mutates: true
---

# generate/sbom

Сгенерировать SBOM (CycloneDX) из lock-файлов проекта — состав зависимостей для CI/supply-chain.

## Как запустить

Через MCP-сервер ailc (один офлайновый бинарь, без внешних сервисов):

- `run { "id": "generate/sbom", "path": "<путь к проекту>" }`
- семантический подбор под намерение: `find_capability { "query": "..." }`

Через CLI:

```
ailc cap generate/sbom <путь>
```

Находки возвращаются структурно (file:line + severity) и проходят состязательную верификацию — в балл и гейт идут только подтверждённые. Семейство `generate`, движок `generator`, тир `core`.
